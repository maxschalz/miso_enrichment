...
EnrichmentCalculatorTest::EnrichmentCalculatorTest() :
  ...
  e = EnrichmentCalculator(compmap_nat_U(), target_product_assay,
...
TEST_F(EnrichmentCalculatorTest, DownblendingOptions) {
  ...
  bool use_integer_stages = true;
  EXPECT_NO_THROW(
    EnrichmentCalculator(compmap_nat_U(), target_product_assay,
                         target_tails_assay, gamma, enrichment_process,
                         target_feed_qty,
                         target_product_qty, max_swu,
                         use_downblending, use_integer_stages)
...
TEST_F(EnrichmentCalculatorTest, AssignmentOperator) {
  ...
  EXPECT_TRUE(misotest::CompareCompMap(tails_comp2,
                                       tails_comp));
...
TEST_F(EnrichmentCalculatorTest, Downblending) {
  ...
  EnrichmentCalculator blender(compmap_nat_U(), target_product_assay, 0.001,
  ...
  // should be identical.
  EnrichmentCalculator blender2(compmap_nat_U(), target_product_assay,
...
TEST_F(EnrichmentCalculatorTest, NonIntegerStagesNumbers) {
  ...
  e2.EnrichmentOutput(pc, tc, feed_qty, swu_used,
                      product_qty, tails_qty, n_enriching, n_stripping);
  EXPECT_TRUE(misotest::CompareCompMap(pc, expected_product_comp));
  EXPECT_TRUE(misotest::CompareCompMap(tc, expected_tails_comp));
}
